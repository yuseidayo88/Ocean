...
export default function TeamPage() {
  ...
  const sel = lines.find((l) => l.id === openId) ?? null;
 ...
 */
function SettingsPane({ who, l, skills, onToggle, onClose }: {
  who: 'employee' | 'exec' | 'all'; l: Line; skills: SkillRow[];
  onToggle: (id: string, on: boolean) => void; onClose: () => void;
}) {
  const { say5 } = useShell();
