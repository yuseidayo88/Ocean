    ...
    <>
      <Centre>
        ...
        <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', padding: '20px 26px 112px', display: 'flex', flexDirection: 'column', gap: 30 }}>
          ...
          {/* 事実の帯 — ラベル（小）→ 数字（大）。数字の下に説明文は置かない */}
          <div style={{ display: 'flex', gap: 24 }}>
            {([
              ['進捗', `${w.progress}%`, undefined, undefined],
              ['フェーズ', `${w.phaseIndex} / ${w.phases.length}`, undefined, w.phases.find((p) => p.state === 'now')!.name],
              ['判断待ち', w.gate ? '1' : '—', w.gate ? AMBER_T : undefined, w.gate?.label],
              ['残り', `${w.restDays}日`, late ? RED_T : undefined, w.endDate],
              ['AI社員', String(w.crew.length), undefined, undefined],
            ] as [string, string, string | undefined, string | undefined][]).map(([k, v, c, sub], i, arr) => (
              <div key={k} style={{
                ...
                {sub && (
                  <span style={{ color: T5, fontSize: 11, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {sub}
                  </span>
                )}
              </div>
                ...
                <span style={{ width: 110, flexShrink: 0 }}>
                  <span style={{ display: 'block', height: 4, borderRadius: 2, background: '#161616', overflow: 'hidden' }}>
                    <span style={{
                      ...
                      background: p.state === 'done' ? '#2E2E2E' : p.state === 'now' ? '#6E6E6E' : 'transparent',
                    }} />
                  </span>
                </span>
          ...
          <div>
            ...
            {dels.map((d, i) => (
              <div key={d.id} style={{
                ...
                borderBottom: i === dels.length - 1 ? undefined : '1px solid #161616',
              }}>
                <Icon name="deliv" color={T4} size={14} />
                ...
                <span style={{ color: T5, fontSize: 12 }}>
                  {employee(d.by).name}{d.when && ` · ${d.when}`}
                </span>
                {d.state === '要確認' && (
                  <span style={{
