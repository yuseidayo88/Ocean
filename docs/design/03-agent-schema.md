...
| **ルール** | 1行ずつ日本語で追記・削除 |
| **モデル** | **自動**（既定）または手動で固定 |
| **思考の深さ** | **自動**（既定）または手動。手動のときは**スライダー**で（左＝速い / 右＝深い） |
...
**思考の深さはスライダー**（参考: Claude Code のエフォート）。粒のグリッドが右へ行くほど
明るくなり、白いつまみで止める。左端が **速い**、右端が **深い**。段階のピルにしない。
自動のときはスライダーを沈めて、いまどこに居るかだけ見せる。
