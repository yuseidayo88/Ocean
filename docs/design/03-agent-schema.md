...
画面は **自動 / 手動** の2択にする（ChatGPT のモデル選択と同じ形）。
「エコ」のような独自の言い方はしない。**既定は自動。**

```yaml
model_policy: auto        # auto | manual
model_fixed: null         # manual のときだけ使う
effort_policy: auto       # auto | low | medium | high
```

手動を選ぶと、こう並ぶ。**モデル名は手動の行にだけ出る。**

| 表示 | 説明 | 実際のモデル |
|---|---|---|
| 自動 | 仕事に合わせて選ばれます | — |
| 高速 | 軽い作業を速く | Haiku 4.5 |
| 標準 | ふだんの仕事 | Sonnet 5 |
| 高精度 | むずかしい判断 | Opus 5 |
...
2. 会社の既定（`accounts.defaults`）
3. 社員ごとの上書き（`employee_settings`）
