...
### 判断ログ（2026-08-25）— 社員ごとにモデルを選べるようにした

**社長の判断: メンバー画面で、社員ごとにモデルと深さを変えられるようにしたい。**
まずは Claude のモデルと OpenAI のモデルで。

- **一覧は1枚**（`lib/ai/catalog.ts`）— Claude 3枚（Opus 5 / Sonnet 5 / Haiku 4.5）と
  OpenAI 3枚（GPT-5.6 Sol / Terra / Luna）。モデルごとに**通り道での名前・直つなぎの名前・
  受ける深さの段・単価**を持つ。画面も実行も原価もここだけを見る
- **`TIER_TABLE` は残す。** 階層（fast / standard / deep）は「どの仕事か」の分類で、
  一覧は「何で走るか」。**選んでいない人は表のモデルで走る**（既定）
- **選んだものが本当に届く**（`RunInput.model` → OpenRouter / Anthropic / OpenAI の3つとも）。
  `runs.model` にも残るので、あとから何で走ったか分かる
- **原価もそのモデルの単価で数える**（`billedCostUsd(tier, in, out, model)`）。
  単価を知らないモデル（無料のテスト・env の差し替え）は 0 のまま
- **深さの段はモデルごとに違う。** Claude は `low`〜`max` の5段（`none` は無い）、
  GPT-5.6 は `none` を含む6段、**Haiku 4.5 は深さを受けない**。
  **受けない段は画面に出さない** — 送るとその往復ごと弾かれる
- **直つなぎのときは、通り道と同じ会社のモデルだけ選ばせる**（`resolve`）。
  OpenAI に直つなぎしている器へ `claude-opus-5` を渡しても通らない
- **確かめていないこと**: 通り道での綴り（`id`）は **`openai/gpt-5.6-luna` だけが実測**。
  残り5枚は各社の名前から組んだもの（この環境から `openrouter.ai` に出られない）。
  鍵が入ったら最初に一覧と突き合わせる（→ `docs/RUNNING.md`）
