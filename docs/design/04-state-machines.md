...
色は Phase 1 のデザイン言語に従う（青＝次に押すもの / 緑＝済・推奨 / 橙＝あなた待ち / 赤＝停止）。
...
`budget_reached` は**終了ではない**。セッションは保持されたまま止まる。
上限を上げれば続きから走る。画面には「クレジット上限で停止中」と、増額ボタンを出す。
