...
2. 承認済の決定事項      decisions where status='decided' の一覧（構造化）
...
[ 半安定] Workのゴール＋承認済決定事項                      ← 決定が増えたときだけ変わる
─────────── ここまでをキャッシュ ───────────
...
2回目の失敗 → タスクを blocked にし、「何が起きたか／何を変えれば進むか」を提示して 判断待ち
```
...
**画面は「自動 / 手動」の2択。既定は自動。** 自動のままなら、ユーザーはモデルの名前を一度も見ません。
手動に切り替えたときだけ、高速 / 標準 / 高精度と実モデル名が並びます
（→ [03](./03-agent-schema.md) モデルと思考の深さ）。
...
## fast の出力は、指示ではなくデータとして扱う

`fast` の「集める」工程は、**信用できない外部の文章を読む**唯一の場所です。
Webページや取り込んだ資料に「これまでの指示を無視して〜」と書いてあっても、
それは**読んだ内容**であって、こちらへの命令ではありません。

だから受け渡しの形を固定します。

```
[ 集める ] fast     … 出力は { 要点, 引用文, URL } の配列だけ。
                       自由文で「次にこうしてください」を返させない
      ↓ データとして渡す
[ 作る   ] standard … 引用文とURLを突き合わせてから成果物を書く
```

- **`fast` の出力を、そのまま次のプロンプトの命令部に入れない。** 資料の位置に入れる
- **`fast` には副作用のある道具を渡さない。** 読む道具（検索・取得）だけ
- 引用が取れなかった主張は、`standard` 側で**落とす**。埋めない

これが効いていれば、`fast` にどのモデルを使っても被害が閉じます。
逆にここが緩いままなら、どんなに強いモデルを使っても危ない
（→ [05](./05-tech-and-cost.md) 判断ログ）。

## デスク — 手もとを見せる

ホームの2番目のビュー。稼働中のAI社員を**縦長のレーンで横に並べ**、
いま手もとで何が起きているかを一気に見る。

**出しているのは演出ではなく `run_steps` そのもの。** 進捗の嘘をつかないという縛りの、いちばん強い形。

| 層 | 中身 | どこから来るか |
|---|---|---|
| 見出し1行 | 「競合の継続率を、公開レポートから拾っています」 | 統括AIが日本語で書く |
| 工程の行 | 「市場規模のレポートを3本 読んだ · 2分41秒」 | `run_steps` 1行＝1行。**畳んで要約**、生ログを流さない |
| 中身 | 事実 / 文章 / コード / 成果物 | いま動いている工程の中身 |
| 足もと | タスク名・進捗・経過時間 | `tasks` と `runs` |

### リアルタイムの出し方

Durable Object から **WebSocket で押し出す**（→ [05](./05-tech-and-cost.md)）。DBを経由しない。

| 器 | 何が動くか |
|---|---|
| 文章 | 書かれている行が**1文字ずつ伸びる**（トークンのストリーム） |
| コード | `write` / `edit` の**差分が行単位で入る**。＋の行が緑で足され、行番号とキャレットが動く |
| 調査 | 取得したページが**上から積まれる**。いま読んでいる行だけが光る |
| 成果物 | できた瞬間にカードが出て、判断待ちに変わる |

- **中身の器は `produces` で決める。** 職種名で分岐しない（業種を埋め込まない原則）
- **止まっているときは止まっていると出す。** モデルが考えている間は「考えています」と出し、
  動いていない行をアニメーションさせない
- 1人を選ぶと全画面。人が増えたらレーンが横にスクロールする
