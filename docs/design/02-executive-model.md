...
| **クレジット上限** | Work 予算 / 会社残高に達したら停止（→ [05](./05-tech-and-cost.md)） |
...
| 用途 | 階層 | 実モデル | 理由 |
|---|---|---|---|
| 統括AI（計画・レビュー・判断整理） | `deep` | Claude Opus 5 | 判断の質が全体を決める |
| AI社員（調査・作成・分析） | `standard` | Claude Sonnet 5 | 量をこなす。読む量が多い |
| 要約・分類・通知文 | `fast` | Claude Haiku 4.5 | 短く安い |

抽象名（`deep / standard / fast`）でコードを書き、実モデルIDは設定表に置く。
画面には出さない。「詳細」の奥にだけ、上書きトグルを置く。
