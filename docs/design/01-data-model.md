...
#### 決まるまでの道（2026-08-25 に整えた）

聞く → 候補 → 選ぶ → 計画、の1本道を、**行き止まりの無い形**にした。

- **3つとも違うときの道を置く。** 候補のカードのいちばん下に「どれも違う — 条件から見直す」。
  押すと会話に戻り、**何が違うかを1問だけ**聞いてから、**違う軸で**出し直す
  （相手 / 売り方 / 分野そのもの）。すぐ3つ並べ替えても、社長には同じものに見える
- **答えは答えであって、新しいゴールではない。** 「質問 → 答え」の行から
  `propose_work` が返ってきて、**質問文がそのまま Work の題**になったことがある。
  探索の途中（候補の道の中で、まだ Work が無い）は、答えの往復からの提案は取らない
- **選んだ道は、いちばん最初の「決めたこと」**として台帳に残す
  （`decisions`。`question='どの道で進めるか'` / 選ばなかった2つも options に）。
  なぜその道かは、**選ばなかった道と並べてはじめて意味になる**。
  この決定は、そのあとの実行の依頼文にも載る（Phase 9 の受け渡し）
- **吹き出しに出るのは社長が決めたことだけ。** 候補のねらいと集めた条件は
  統括AIにだけ渡す（`startWork(goal, ctx)`）。前は全部つないで1つのゴールにしていたので、
  機械が組んだ長い1文が「社長が書いた言葉」として画面に出ていた

### 会話の考え方（2026-08-25 に一本に締め直した）

**元になっている1つの考え** — 社長がここに書くのは「やりたいこと」か「困っていること」で、
**どちらも、そのままでは仕事にならない**。会話の仕事は、それを**押せる形**
（質問カード / 候補3つ / 診断 / Work の提案）に変えること。
だから**統括AIの番は、次に押せるものか、答えそのもので終わる**。
文章だけで終わっていい往復は、**社長が何かを尋ねたとき**だけ。

本文は `lib/exec/chat.ts` の `GUIDE`（憲法と同じで、コードの中が正本）。決めごとは4つ:

| | |
|---|---|
| **段を毎回そのまま言う** | `stageLine` が「いまどこにいるか」を1行で渡す。状態の断片から毎回推測させると外れる（条件を書き留めただけで止まる、同じことを聞き直す、が実際に起きた）。探索中は**まだ聞けていない条件の名前**まで出すので、次に何を聞くかが決まる |
| **答えの往復は1回で済ませる** | 「質問 → 答え」が届いた往復は、**写すのも次の一手も同じ1回**（道具は複数呼べる）。前は写す往復と出す往復に分けていて、1つの発言にモデルを3回呼んでいた。続きの輪（`reply.ts`）は保険として残す |
| **最初の一言は消さない** | history は直近10件＋**先頭固定**。探索が長引くと「そもそも何と言って始めたか」が消えていた |
| **待たせているものを知っている** | `snapshot` に判断待ちと要確認の件数。聞かれていなくても一言添える（放っておくと会社が止まるところ） |
