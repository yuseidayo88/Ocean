...
export type LiveWork = {
  ...
  phases: {
    ...
    owner?: string;
    /**
     * **このフェーズの終わりに、社長が決めること**（計画の ◆。`plan_draft` の `gates`）。
     *
     * 計画の画面は軸の上に ◆ を立てて「あなたが決めるのは ◆ の N か所」と言い、
     * 社長はそれを読んで承認する。それなのに**承認したあとの Work 画面には
     * どこにも出ていなかった** — いつ自分の番が来るのかが、承認した瞬間に分からなくなる。
     */
    gate?: string;
    /**
