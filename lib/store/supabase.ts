...
export const supabaseStore: Store = {
  ...
  async getDiscovery(id) {
    ...
    const { data: rows } = await c.from('discovery_candidates')
      .select('id, name, summary, ending, fit, why, recommended, not_chosen_why, adopted_work_id, who, first_make, first_one, unsure, hours_per_week, created_at')
    ...
    return {
      ...
      candidates: sortCands(latest.map((r) => ({
        ...
        firstMake: Array.isArray(r.first_make) && r.first_make.length
          ? (r.first_make as string[]).map(String) : undefined,
  ...
  async setCandidates(id, cands) {
    ...
    const { error } = await c.from('discovery_candidates').insert(cands.map((x) => ({
      ...
      who: x.who ?? '', first_make: x.firstMake ?? [], first_one: x.firstOne ?? '', unsure: x.unsure ?? '',
