...
export const proposeCandidates: ToolDef = {
  ...
  description:
    ...
    + '（範囲を狭めて、時間の中に収まる形にする）。'
    + '\n**3つは違う道にする。** 同じ商売の値段違い・規模違いを3つ並べても、'
    + '社長は選べません。**売るものが違う / 相手が違う / 届け方が違う** のどれかで分ける。'
    + '\n**行き先だけ書かない。** first_make（会社が最初に作るもの）が書けないと、'
    + '社長は3つを比べられません — どれも「良さそうな事業の説明」で終わります。'
        ...
        items: {
          ...
          properties: {
            ...
            first_make: {
              type: 'array',
              items: { type: 'string' },
              description: '**この案を選んだら、会社が最初に作るもの**を2〜3件。'
                + '**作るものの名前**で書く（「飲食店むけメニュー表のひな形3種」'
                + '「申し込みページ1枚」「価格表」）。'
                + '**「調査する」「検討する」「設計する」は作るものではありません** — '
                + '社長の手もとに残るものだけを書く。'
                + '社長はここを見て3つを比べます（行き先が似ていても、やることは違う）',
            },
            first_one: {
              type: 'string',
