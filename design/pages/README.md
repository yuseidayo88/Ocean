...
**1行が2段。** 上が名前の段、下が「約束 ＋ できること」の段。

    ● 統括AI  Executive  実行中                              Opus 5 ⌄     ⚌
      あなたの言葉は全部ここに届きます。 [Workを立てる][計画を作る][社員を選ぶ] +1  深さ ····▮·

- **上の段は誰か、下の段は何をしてくれるか。** 前は3段で、下に余白が余っていた
- **約束は 290px の固定列。** できることの始まりが全行で揃うので、
  横に読んでも縦に読んでも表になる。列がずれると「似た行が5本」に見える
...
| 深さ のラベル | **992** |
| モデル名と深さの右端 | **1110** |
| つまみのアイコンの右端 / すべての右端 | **1150** |

- つまみのアイコンは**モデルと深さの縦積みの中心**に置く（前は上の1行に寄っていた）
