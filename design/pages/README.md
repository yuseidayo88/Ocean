...
- **モデルは小さく。** 素の文字＋⌄ だけ（自動 ＋ Haiku 4.5 / Sonnet 5 / Opus 5）。
  自動/手動のトグルはやめた。押したときだけ面が出る
- **スキルとルールの中身は設定へ。** 各社員の行に `スキルとルールを設定 ›`。
  面に出すのは「できること」の名前だけ
- **思考の深さはスライダー**（参考: Claude Code のエフォート）。
  目盛りに「どこからどのモデルになるか」を出す ── `tierFor(effort)` が実際にやっていることを
  そのまま見せる（自動 ＝ 深さがモデルを選ぶ）
