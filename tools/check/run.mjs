...
/**
 * **引き直しても残ったところは、正直に言う**（2026-08-26。図の作法と同じ）。
...
/**
 * **承認した計画の担当に、本当に仕事が行く**（2026-08-26。社長の
 * 「AI同士が成果物を引き継いで、各自何すればいいか考えてやってました？」）。
 *
 * 計画は「プロダクト = 開発担当」と書いて、社長はそれを承認している。
 * ところが `draftNextTasks` に渡していたのは **フェーズ名とゴールだけ**で、
 * **担当は渡っていなかった** — だから統括AIは誰でも選べて、
 * 採用したのに一度も働かない社員ができる（ロゴの例で実際そうなった）。
 */
const owned = await until((b) => /MVPの要件を書く/.test(b), 60);
ok('承認した計画の担当に、次のフェーズの仕事が行く',
   /MVPの要件を書く[\s\S]{0,40}開発担当/.test(owned) || /開発担当[\s\S]{0,40}MVPの要件を書く/.test(owned),
   owned.match(/MVPの要件を書く[\s\S]{0,40}/)?.[0]?.replace(/\n/g, ' ') ?? '(出ていない)');
...
ok('候補が「何ができたら完了か」を出している', cands.includes('完了') && cands.includes('受け終わっている'),
   cands.slice(-240));
/**
 * **候補は「実際に需要があって、1人で回せる仕事」で選ぶ**（2026-08-26。社長の指示）。
 * 前の軸は 速さ / 安さ / 得意との相性で、**需要も「1人で回せるか」も入っていなかった** —
 * だから抽象的で当たり障りのない案に寄っていた。
 */
ok('候補の軸が 需要 / 1人で回せる になっている',
   cands.includes('欲しい人がいる') && cands.includes('1人で回せる') && !cands.includes('初期費用の低さ'),
   cands.match(/欲しい人がいる[\s\S]{0,40}/)?.[0]?.replace(/\n/g, ' ') ?? cands.slice(-200));
/**
 * **「これを選んだら明日から何をするのか」が読める**（2026-08-28。社長の
 * 「なんかプランの提案が微妙、何をしたいのかがわかりにくい」）。
 * 前は行き先しか書いていなかった — 完了・誰が買うか・相性。
 * **行き先が同じくらい良さそうな3案は、やることが違うから選べる。**
 */
ok('選んだら会社が最初に作るものが書いてある',
   cands.includes('まず作る') && cands.includes('申し込みページ1枚'),
   cands.match(/まず作る[\s\S]{0,60}/)?.[0]?.replace(/\n/g, ' ') ?? cands.slice(-200));
// **抽象に逃げない。** 誰が買うのか・最初の1人をどこで見つけるかが書けない候補は、候補ではない
ok('誰が買うのか、最初の1人をどこで見つけるかが書いてある',
   cands.includes('誰が') && cands.includes('最初の1人') && cands.includes('直接声をかける'),
   cands.match(/最初の1人[\s\S]{0,50}/)?.[0]?.replace(/\n/g, ' ') ?? cands.slice(-200));
/**
 * **確かめていないことを、候補の顔に出す。**
 * 統括AIは Web を見ていないので、需要は記憶から言っているだけ —
 * **黙って自信ありげに出すほうが危ない**（AI社員の憲法の「未確認」と同じ作法）。
 */
ok('まだ確かめていないことを、候補が自分から言う',
   cands.includes('まだ確かめていない') && /週およそ \d+時間/.test(cands),
   cands.match(/まだ確かめていない[^\n]*/)?.[0] ?? cands.slice(-200));
